    ...
    public void click() {
        if (wrappedElement != null) {
            ...
            if (wrappedElement.getTagName().equals("a")) {
                if (Driver.getCurrentDriver().getClass().getSimpleName().contains("Explorer")) {
                	try {
                		  Driver.getCurrentDriver()
                                  .executeScript("$(arguments[0]).focus();", wrappedElement);
					} catch (Exception ignored) {
					}
                    wrappedElement.click();
                    Logger.logDebug("we passed to:"+ Driver.getCurrentDriver().getCurrentUrl());

                    Logger.logInfo(Localization
                            .getMessage(Localization.CLICK_BUTTON, name, page));
                    return;
                }
                Driver.getCurrentDriver()
                        .executeScript("$(arguments[0]).focus();", wrappedElement);
            ...
            Logger.logError(Localization
                    .getMessage(Localization.NO_BUTTON, name));
    }
    ...
    public void clickJquery() {
        if (wrappedElement != null) {
            ...
            Logger.logDebug("we passed to:"
                    + Driver.getCurrentDriver().getCurrentUrl());
            Logger.logInfo(Localization
                    .getMessage(Localization.CLICK_JQUERY, name, page));
        } else
            Logger.logError(Localization
                    .getMessage(Localization.NO_BUTTON, name));
    }
