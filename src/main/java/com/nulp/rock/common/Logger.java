...
import com.nulp.rock.logging.LogMarkers;
...
public class Logger {
    ...
    public static String makeScreenshot(String fileName) {
        ...
        try {
            if (Driver.getCurrentDriver().getClass().getSimpleName().contains("Remote")) {return null;}
    ...
    public static void logSubClose() {
        if (tabPrint.length() >= 7) {
            ...
            System.out.println("You need call function 'logSubOpen()' before call  function 'logSubClose()'");
        }
    }
    ...
    public static void logFail(String message) {
        ...
        if (!Driver.getCurrentDriver().getClass().getSimpleName().contains("Remote")
                && !Driver.getCurrentDriver().getClass().getSimpleName().contains("Chrome")) {
        }
        if (isTest) {
            hideHtmlDetails();
        }
